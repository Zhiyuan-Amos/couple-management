...
public class DoneStateContainer : Notifier
{
    ...
    public IReadOnlyDictionary<DateOnly, IReadOnlyList<object>> GetDateToItems()
    {
        return _dateToItems;
    }
    ...
    {
        _dateToItems.Clear();
        foreach (var (key, value) in toSet) _dateToItems.Add(key, value);
    ...
    {
        if (!_idToImage.TryGetValue(id, out image)) return false;
